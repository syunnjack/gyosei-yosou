...
import { CalendarDays, CheckCircle2, ChevronRight, ClipboardList, Flag, HelpCircle, ShieldCheck, TrendingUp } from 'lucide-react'
import { EXAM_DATE, MAX_SCORE, PASS_LINE, faq, isPass, lawOf, mockExams, providers, totalOf } from './mockExams.js'
  ...
  useEffect(() => {
    ...
    el.text = JSON.stringify([
      { '@context': 'https://schema.org', '@type': 'Blog', name: '行政書士試験 模試の記録', url: SITE + '#mock-exams', inLanguage: 'ja',
        description: '令和8年度（2026年11月8日）行政書士試験に向けた模試13回分の得点推移と復習記録。',
...
export default function MockExams() {
  ...
  return <div className="content" id="mock-exams">
    <section className="hero mock-hero"><div><span className="tag"><Flag />MOCK EXAM LOG</span><h2>合格までの推移を、<br /><em>模試13回で記録する。</em></h2><p>令和8年度 行政書士試験（{fmt(EXAM_DATE)}）まで、LEC・伊藤塾・TAC・東京法経学院の模試を受験。総得点だけでなく法令122点・基礎知識24点の足切りも毎回チェックし、次回までの復習項目を残します。</p></div><div className="score-ring countdown"><span>本試験まで</span><strong>{daysLeft()}<small>日</small></strong><p>{fmt(EXAM_DATE)}</p></div></section>
    ...
    <section className="exam-list">{list.map(e => { const p = providerOf(e.provider); const s = e.scores; const isOpen = open === e.id
      return <article key={e.id} id={`mock-${e.id}`} className={`exam ${e.status}`}>
        <button className="exam-head" onClick={() => setOpen(isOpen ? null : e.id)} aria-expanded={isOpen}>
          ...
          <div><h3>{e.title}</h3><time dateTime={e.date || undefined}>{fmt(e.date)}</time></div>
          {s ? <div className={`total ${isPass(s) ? 'ok' : ''}`}><strong>{totalOf(s)}</strong><small>/300</small><em>{isPass(s) ? '合格圏' : '合格ライン未達'}</em></div> : <div className="total planned"><em>受験予定</em></div>}

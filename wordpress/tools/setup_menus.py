...
def menu(name, items, locations):
    ...
    have = {i['title']['rendered'] for i in s.get(A + f'/menu-items?menus={mid}&per_page=100', headers=H).json()}
    for i, (title, kind, key) in enumerate(items):
        if title in have:
            continue
        body = {'title': title, 'menus': mid, 'status': 'publish', 'menu_order': i + 1}
        if kind == 'page':
            body.update({'type': 'post_type', 'object': 'page', 'object_id': pages[key]})
        else:
            body.update({'type': 'taxonomy', 'object': 'category', 'object_id': cats[key]})
